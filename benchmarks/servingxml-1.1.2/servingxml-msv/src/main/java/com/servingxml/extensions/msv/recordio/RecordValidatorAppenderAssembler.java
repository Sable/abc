...
public class RecordValidatorAppenderAssembler {
  ...
  public RecordFilterAppender assemble(ConfigurationContext context) {
    ...
    //  revisit
    if (cache == null) {
      recordFilterAppender = new RecordValidatorAppender(env, xmlSourceFactory, errorHandler);
    } else {
      recordFilterAppender = new CachedRecordValidatorAppender(env, xmlSourceFactory,
                                                               cache, 
                                                               expiryOptions.getRevalidationType(),
                                                               errorHandler);
    }
