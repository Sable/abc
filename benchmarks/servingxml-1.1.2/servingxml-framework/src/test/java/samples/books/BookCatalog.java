...
public class BookCatalog implements DynamicContentHandler, Cacheable {
  private static final String sourceClass = BookCatalog.class.getName();
  ...
  ContentWriter contentWriter) {
    ...
    context.trace(sourceClass,sourceMethod,"parameters = " + parameters.getCategory(),Level.FINEST);
    ...
    context.trace(sourceClass,sourceMethod,"category = " + categoryCode,Level.FINEST);
    if (categoryCode.length() > 0) {
      Iterator iter = bookList.iterator();
      while (iter.hasNext()) {
        BookEntry bookEntry = (BookEntry)iter.next();
        if (bookEntry.getCategoryCode().equals(categoryCode)) {
          context.trace(sourceClass,sourceMethod,"processing = " + categoryCode,Level.FINEST);
