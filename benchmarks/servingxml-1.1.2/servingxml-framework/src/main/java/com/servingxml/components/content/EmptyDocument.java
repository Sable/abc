...
/**
 * A <code>EmptyDocument</code> instance may be used to obtain objects that
...
public class EmptyDocument extends AbstractContent implements Content {     
  ...
  public SaxSource createSaxSource(ServiceContext context, Flow flow) {
    return new EmptySaxSource(context.getTransformerFactory());
  }                      

  public void appendToXmlPipeline(ServiceContext context, Flow flow, XmlFilterChain pipeline) {
    SaxSource saxSource = new EmptySaxSource(context.getTransformerFactory());
