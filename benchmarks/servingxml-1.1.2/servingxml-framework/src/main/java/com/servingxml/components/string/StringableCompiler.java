...
/**
 * The <code>StringableCompiler</code> implements a factory for
 * creating <code>StringEvaluator</code> objects.
 *
 * 
...
public class StringableCompiler {
  ...
  public static StringFactory compile(ConfigurationContext context, Element textElement) {
    //System.out.println("StringableCompiler.compile " + context.getElement().getTagName());

    TextElementCommand command = new TextElementCommand(context, null);
  ...
  static class TextElementCommand extends DomIterator.ChildCommand {
    ...
    TextElementCommand(ConfigurationContext context, StringFactory tail) {
    ...
    public void doElement(Element contextElement, Element componentElement) {
      ...
      if (component != null) {
        ...
        if (Stringable.class.isAssignableFrom(component)) {
