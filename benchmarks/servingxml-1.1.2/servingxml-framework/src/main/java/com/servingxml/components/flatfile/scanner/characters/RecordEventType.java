...
import com.servingxml.components.flatfile.scanner.characters.CharRecordInput;
...
abstract class RecordEventType {
  ...
  abstract void write(char[] bytes, int start, int length, Charset charset, FlatContentReceiver receiver);

  abstract boolean isEmptyLine(char[] bytes, int start, int length, CharTrimmer byteTrimmer);

  static final class StartLineEventType extends RecordEventType {
    final void write(char[] buffer, int start, int length, Charset charset, FlatContentReceiver receiver) {
    }

    final boolean isEmptyLine(char[] bytes, int start, int length, CharTrimmer byteTrimmer) {
      return true;
    }
  }
  ...
  static final class DataEventType extends RecordEventType {
    final void write(char[] buffer, int start, int length, Charset charset, FlatContentReceiver receiver) {
      RecordInput recordInput = new CharRecordInput(buffer, start, length, charset);
