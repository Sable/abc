...
public class CharFlatFilePostprocessor implements FlatFilePostprocessor {
  private final StreamSink sink;
  private final Writer writer; 
  private final boolean flushRecordOnWrite;

  public CharFlatFilePostprocessor(StreamSink sink, boolean flushRecordOnWrite) {
    this.sink = sink;
    this.flushRecordOnWrite = flushRecordOnWrite;

    OutputStream os = sink.getOutputStream();
    Charset charset = sink.getCharset();
    Writer w = (charset == null) ? new OutputStreamWriter(os) : new OutputStreamWriter(os, charset);
    writer = new BufferedWriter(w);
  }

  public void write(RecordOutput recordOutput)
  throws IOException {
    char[] data = recordOutput.toCharArray();
    writer.write(data, 0, data.length);
    if (flushRecordOnWrite) {
      //System.out.println("FLUSHING RECORD");
      writer.flush();
    }
  }
