...
public final class MultipleCommentStarterChecker extends CommentStarterByteChecker {

  private final CommentStarterByteChecker[] commentStarterCheckers;
  ...
  throws IOException {
    ...
    for (int i = 0; !done && i < commentStarterCheckers.length; ++i) {
      done = commentStarterCheckers[i].checkCommentSymbol(recordBuffer,byteArrayBuilder);
    }
    return done;
  }
}
