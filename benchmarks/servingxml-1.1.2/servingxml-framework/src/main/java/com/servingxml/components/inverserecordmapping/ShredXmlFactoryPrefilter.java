...
public class ShredXmlFactoryPrefilter implements ShredXmlFactory {
  private final ShredXmlFactory flattenerFactory;
  private final ParameterDescriptor[] parameterDescriptors;

  public ShredXmlFactoryPrefilter(ShredXmlFactory flattenerFactory,
  ...
  public ShredXml createShredXml(ServiceContext context, Flow flow) {
    ...
    ShredXml flattener = flattenerFactory.createShredXml(context, newFlow);
    return new ShredXmlPrefilter(flattener, parameterDescriptors);
  }

  public SubtreeFieldMap createSubtreeFieldMap(Name fieldName, String matchExpr) {
    return flattenerFactory.createSubtreeFieldMap(fieldName, matchExpr);
  }
}
