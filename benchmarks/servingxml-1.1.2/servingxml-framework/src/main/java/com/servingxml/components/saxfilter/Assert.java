...
public class Assert extends XMLFilterImpl implements XMLFilter {
  ...
  public Assert(ServiceContext context, Flow flow,
    ...
    try {
      ...
      if (bufferBuilder != null) {
        ...
        if (level == 0) {
          ...
          if (!testExpr.evaluate(xmlSource, flow.getParameters())) {
            String message = messageFactory.createString(context,flow);
