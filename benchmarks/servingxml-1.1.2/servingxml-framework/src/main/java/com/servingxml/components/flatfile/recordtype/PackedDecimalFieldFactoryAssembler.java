...
public class PackedDecimalFieldFactoryAssembler extends FlatFileOptionsFactoryAssembler {
  ...
  public FlatRecordFieldFactory assemble(ConfigurationContext context) {
    ...
    if (digitCount <= 0) {
      String message = MessageFormatter.getInstance().getMessage(ServingXmlMessages.COMPONENT_ATTRIBUTE_REQUIRED,
                         context.getElement().getTagName(),"digitCount");
      throw new ServingXmlException(message);
    }
    ...
    FlatRecordFieldFactory fieldTypeFactory = new PackedDecimalFieldFactory(fieldName,label,
                                                start,width,digitCount,decimalPlaces,
                                                defaultValue,flatFileOptionsFactory);
