...
// $Id: PutListResource.java,v 1.13 2000/08/16 21:37:32 ylafon Exp $
...
public class PutListResource extends FramedResource {
	...
	Enumeration _enum = published.elements();
	PutedEntry oldest = null;
	PutedEntry current = null;

	if (_enum.hasMoreElements())
	    oldest = (PutedEntry) _enum.nextElement();
	else
	    return;
	while (_enum.hasMoreElements()) {
	    current = (PutedEntry) _enum.nextElement();
    ...
    protected synchronized void write(File file, Enumeration _enum) {
	if (file == null)
	    return;
	File backup = null;
	...
	Enumeration _enum = getDelEntries();
	while (_enum.hasMoreElements()) {
	    DeletedEntry e = (DeletedEntry) _enum.nextElement();
	    if (! e.isConfirmed())
