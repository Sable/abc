...
// $Id: PutListFrame.java,v 1.10 2000/08/16 21:37:32 ylafon Exp $
...
public class PutListFrame extends PostableFrame {
	...
	Enumeration _enum = putlist.getPublishedEntries();
	if (! _enum.hasMoreElements())
	    g.append("<center><b>No recently published file</b></center>\n");
	g.append("<ul>\n");
	
	Vector sorted = 
	    org.w3c.tools.sorter.Sorter.sortComparableEnumeration(_enum);
	...
	if (action.equals("config")) {
		...
		_enum = putlist.getEntriesKeys();
	    else
		_enum = putlist.getDelEntriesKeys();
