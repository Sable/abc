...
// $Id: CacheGeneration.java,v 1.36 2003/02/14 16:25:01 ylafon Exp $
...
public class CacheGeneration implements LRUAble {
	...
	if (loaded) {
	    Enumeration _enum = lookupTable.elements();
	    while (_enum.hasMoreElements()) {
		CachedResource cr = (CachedResource) _enum.nextElement();
	    ...
	    Enumeration _enum = lookupTable.elements();
	    while (_enum.hasMoreElements()) {
		String file = (String) _enum.nextElement();
		if (! file.equals("")) {
	...
	bytecount        = 0;
	cr_count         = 0;
	Enumeration _enum = lookupTable.keys();
	while(_enum.hasMoreElements()) {
	    lookupTable.put(_enum.nextElement(), Boolean.TRUE);
	}
    }
