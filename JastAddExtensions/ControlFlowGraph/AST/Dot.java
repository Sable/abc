...
public class Dot extends AbstractDot implements Cloneable {
  ...
  public boolean mayHaveRewrite() { return true; }
    ...
    // Declared in ControlFlowGraph.jrag at line 137
 @SuppressWarnings({"unchecked", "cast"})     public Expr first() {
        Expr first_value = first_compute();
        return first_value;
    }

    private Expr first_compute() {  return getLeft().first();  }

    // Declared in ControlFlowGraph.jrag at line 139
    public SmallSet Define_SmallSet_following(ASTNode caller, ASTNode child) {
        ...
        if(caller == getLeftNoTransform()) {
            return SmallSet.empty().union(getRight().first());
        }
