...
%state STRING, CHARLITERAL, JASTADD

%%

<JASTADD> {
  "syn" { return newSymbol(Terminals.SYN); }
  "inh" { return newSymbol(Terminals.INH); }
  "lazy" { return newSymbol(Terminals.LAZY); }
  "eq" { return newSymbol(Terminals.EQUATION); }
  "ast" { return newSymbol(Terminals.AST); }
  "::=" { return newSymbol(Terminals.ASTEQ); }
  "rewrite" { return newSymbol(Terminals.REWRITE); }
  "when" { return newSymbol(Terminals.WHEN); }
  "to" { return newSymbol(Terminals.TO); }
  "circular" { return newSymbol(Terminals.CIRCULAR); }
  "sons" { return newSymbol(Terminals.PUBLIC); }
}
<YYINITIAL,JASTADD> {

  /* keywords */
  "aspect"                       { enterJastAdd(); return newSymbol(Terminals.ASPECT); }
  ...
  "class"                        { enterJava(); return newSymbol(Terminals.CLASS); }
...
/*  "enum"                         { return newSymbol(Terminals.ENUM); }*/
  ...
  "interface"                    { enterJava(); return newSymbol(Terminals.INTERFACE); }
...
/*  "goto"                         { return newSymbol(Terminals.GOTO); }*/
  ...
  "{"                            { openBrace(); return newSymbol(Terminals.LBRACE); }
  "}"                            { closeBrace(); return newSymbol(Terminals.RBRACE); }
...
/*  "..."                          { return newSymbol(Terminals.ELLIPSIS); }*/
  ...
  /* string literal */
  \"                             { enterTempState(STRING); string.setLength(0); }

  /* character literal */
  \'                             { enterTempState(CHARLITERAL); }
...
<STRING> {
  \"                             { exitTempState(); return newSymbol(Terminals.STRING_LITERAL, string.toString()); }
...
<CHARLITERAL> {
  {SingleCharacter}\'            { exitTempState(); return newSymbol(Terminals.CHARACTER_LITERAL, new Character(yytext().charAt(0)).toString()); }

  /* escape sequences */
  "\\b"\'                        { exitTempState(); return newSymbol(Terminals.CHARACTER_LITERAL, new Character('\b').toString());}
  "\\t"\'                        { exitTempState(); return newSymbol(Terminals.CHARACTER_LITERAL, new Character('\t').toString());}
  "\\n"\'                        { exitTempState(); return newSymbol(Terminals.CHARACTER_LITERAL, new Character('\n').toString());}
  "\\f"\'                        { exitTempState(); return newSymbol(Terminals.CHARACTER_LITERAL, new Character('\f').toString());}
  "\\r"\'                        { exitTempState(); return newSymbol(Terminals.CHARACTER_LITERAL, new Character('\r').toString());}
  "\\\""\'                       { exitTempState(); return newSymbol(Terminals.CHARACTER_LITERAL, new Character('\"').toString());}
  "\\'"\'                        { exitTempState(); return newSymbol(Terminals.CHARACTER_LITERAL, new Character('\'').toString());}
  "\\\\"\'                       { exitTempState(); return newSymbol(Terminals.CHARACTER_LITERAL, new Character('\\').toString()); }
  \\[0-3]?{OctDigit}?{OctDigit}\' { exitTempState();
