...
Having multiple instances of modules also brings up the issue of the consistency of 
dependencies shared between two modules. For example, if modules \textit{A}
and \textit{B} both depend on \textit{C} and are expected to share types from \texttt{C}, 
they should depend on the exact same instance of \texttt{C}. Otherwise, the shared types
would not be compatible, as they are from different class spaces (i.e. loaded by different classloaders in Java).