...
public class DummyAdvice {
	...
	public static int[] implicitParameters(List<ParameterDeclaration> parameterList){
		int[] params = new int[3];
		for (int i = 0; i < 3; i++) {
			params[i] = -1;

			for (int j = 0; j < parameterList.getNumChild(); j++)
				if (parameterList.getChild(j).name().equals(AdviceDecl.implicitVarName(i)))
					params[i] = j;
		}
		return params;
	}
	
	public static MethodSig proceedSig(Type returnType, List<ParameterDeclaration> parameterDeclaration){ 	
	    return new MethodSig(
	    	      soot.Modifier.PUBLIC | soot.Modifier.FINAL | soot.Modifier.STATIC,
	    	      AbcFactory.AbcClass(DummyAspect.getDummyAspectSootClass()),
	    	      AbcFactory.AbcType(returnType),
	    	      proceedName(),
	    	      proceedFormals(parameterDeclaration),
	    	      new ArrayList(),
	    	      new Position(""));
	}

	public static MethodSig methodSig(Type returnType, List<ParameterDeclaration> parameterDeclaration) {
		    return new MethodSig(
		      1,
		      AbcFactory.AbcClass(DummyAspect.getDummyAspectSootClass()),
		      AbcFactory.AbcType(returnType),
	...
	public static java.util.List proceedFormals(List<ParameterDeclaration> parameterDeclaration){
	...
	public static abc.weaving.aspectinfo.AdviceSpec adviceSpec(Type returnType, List<ParameterDeclaration> parameterDeclaration, TypeAccess jpi){
	    ...
	    return new CJPAroundAdvice(return_type, proceedMethodSig, proceedMethodSig.getPosition(), jpi);
	}
	
	public static SootMethod createProceedMethod(List<ParameterDeclaration> parameterDeclaration){
	    proceedMethod = new SootMethod(proceedMethodSig.getName(), proceedParamList(parameterDeclaration),
	    		proceedMethodSig.getReturnType().getSootType(), proceedMethodSig.getModifiers(),
	                        new ArrayList());
	    return proceedMethod;
		
	}
	
	public static java.util.List proceedParamList(List<ParameterDeclaration> parameterDeclaration){
		ArrayList list = new ArrayList();
	    for(int i = 0; i < parameterDeclaration.getNumChild(); i++)
	...
	public static void jimplify2(List<ParameterDeclaration> parameterDeclaration, TypeDecl returnType, Value returnValue) {
        ...
        LineNumberTag lineTag = new soot.tagkit.LineNumberTag(-1); //-1 -->node.lineNumber()
        //false! --> if(!body.getMethod().isStatic())
        //emitThis(typeDecl);
        //emiting this
        soot.RefType aspectSootType = DummyAspect.getDummyAspectSootClass().getType();
        soot.Local thisName = newLocal("this", aspectSootType, body);
        addStmt(Jimple.v().newIdentityStmt(thisName, Jimple.v().newThisRef(aspectSootType)), lineTag, chains);
        ...
        for(ParameterDeclaration pd : parameterDeclaration){
			...
			addStmt(newIdentityStmt(local,newParameterRef(pd.type().getSootType(), localNum++/*pd.localNum()*/, pd),pd),lineTag, chains);
        }
        if (!returnType.isVoid()){ 
			int nextTempIndex = 0;
			Local local = newLocal("temp$" + nextTempIndex++, returnType.getSootType(), body);
			soot.jimple.AssignStmt assignStmt = Jimple.v().newAssignStmt(local, returnValue);
			soot.tagkit.Tag left = tagMap.get(local);
			if(left != null) assignStmt.getLeftOpBox().addTag(left);
			soot.tagkit.Tag right = tagMap.get(returnValue);
			if(right != null) assignStmt.getRightOpBox().addTag(right);
			...
			if(tag != null) returnStmt.getOpBox().addTag(tag);
        ...
        for(ParameterDeclaration pd : parameterDeclaration){
			...
			addStmt(newIdentityStmt(tempLocal,newParameterRef(pd.type().getSootType(), localNum++/*pd.localNum()*/, pd),pd),lineTag, chains);
        }
	...
	private static Local newLocal(String name, soot.Type type, JimpleBody body) {
  	...
  	private static void addStmt(soot.jimple.Stmt stmt, LineNumberTag lineTag, Stack chains) {
	  stmt.addTag(lineTag);
    ...
    public static soot.jimple.ParameterRef newParameterRef(Type paramType, int number, ASTNode location) {
    ...
    public static soot.jimple.IdentityStmt newIdentityStmt(Value local, Value identityRef, ASTNode location) {
