...
public class DummyAspect {
        ...
        /***
         * Creates an aspect in the front end.  The user must add this
         * aspect in globalinfo structure.
         * @param aspectName
         */
        public static boolean createDummyAspect(String aspectName){
                if (dummyAspect==null){
                    ...
                    dummyAspectSootClass.setSuperclass(Scene.v().getObjectType().getSootClass());
                        dummyAspect = new Aspect(AbcFactory.AbcClass(dummyAspectSootClass), null, new Position(""));                    
                        //jimplify1phase2
                ...
                MethodDecl_jimplify("aspectOf",9,dummyAspectSootClass.getType(),soot.jimple.NullConstant.v(),dummyAspectSootClass.getType());           
                MethodDecl_jimplify("hasAspect",9,soot.BooleanType.v(),BooleanType.emitConstant(true),dummyAspectSootClass.getType());
        ...
        public static void ConstructorDecl_jimplify(String name, RefType aspectSootType){
            ...
            //if(!hostType().getSootClassDecl().declaresMethod(signature)) { we always need the constructor
        SootMethod sootMethod = new SootMethod(name, parameters, returnType, modifiers, throwtypes);
        dummyAspectSootClass.addMethod(sootMethod); //dummyAspectSootClass --> hostType().getSootClassDecl()
        ...
        //!!Body b = new Body(hostType(), body, this);//new Body(hostType(), body, this);
        ...
        SootMethodRef methodRef = Scene.v().makeConstructorRef(dummyAspectSootClass.getSuperclass(), parameters);
        soot.jimple.SpecialInvokeExpr specialInovkeExpr = Jimple.v().newSpecialInvokeExpr(thisName, methodRef, parameters);
        java.util.HashMap<soot.Value, soot.tagkit.Tag> tagMap = new java.util.HashMap<soot.Value, soot.tagkit.Tag>();
        tagMap.put(specialInovkeExpr, new soot.tagkit.LineNumberTag(-1)); //-1 -->node.lineNumber()
        //if(value instanceof soot.jimple.InvokeExpr) {
        //    b.add(b.newInvokeStmt((soot.jimple.InvokeExpr)value, this));
        soot.jimple.InvokeStmt stmt = Jimple.v().newInvokeStmt(specialInovkeExpr);
      ...
      chain.add(stmt);
        ...
        /***
         * Creates and registers a SootMethod and then It does the jimplify process properly (1_phase2, 2)
         * @param name
         * @param typeModifier
         * @param returnType
         * @param returnValue
         ...
         */
        public static void MethodDecl_jimplify(String name, int typeModifier, soot.Type returnType, Value returnValue, RefType aspectSootType){
                ArrayList parameters = new ArrayList();
                ArrayList paramnames = new ArrayList();
                //for(int i = 0; i < getNumParameter(); i++) {
                //  parameters.add(getParameter(i).type().getSootType());
