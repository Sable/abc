 ...
 */
public class ThisVarInv extends ThisAny {
	protected Var var;
	protected WeavingVar weavingVar;

    public ThisVarInv(Var var,Position pos) {
	super(pos);
	this.var = var;
    }
    ...
    protected Residue matchesAt(WeavingEnv we,ContextValue cv) {
        if (CheckType.construct(cv,we.getAbcType(var).getSootType()) == NeverMatch.v())
        	return NeverMatch.v();     	
	...
	else return new ThisVarInv(var,getPosition());
    }
...
//			Var othervar = ((ThisVarInv)otherpc).getVar();
...
//					return true;
//				} else {
//					unification.setPointcut(new ThisVarInv(unifiedvar, unifiedvar.getPosition()));
//					return true;
//				} 
//				}
//			} else return false;
//		} else if (otherpc.getClass() == ThisTypeInv.class) {
...
//			ThisTypeInv othertt = (ThisTypeInv)otherpc;
