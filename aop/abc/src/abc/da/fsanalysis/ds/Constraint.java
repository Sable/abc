...
public class Constraint implements Cloneable {
	...
	public Constraint addBindingsForSymbol(Collection allVariables, SMNode from, SMNode to, Map bindings, Shadow shadow) {
		...
		for (Iterator iter = disjuncts.iterator(); iter.hasNext();) {
			...
			Disjunct newDisjunct = disjunct.addBindingsForSymbol(allVariables,bindings,shadow, from==to);
    ...
    public Constraint applyDistributiveLaw() {
        ...
        while(true) {
	    	...
	    	Object superFlousKey=null;
	    	Disjunct first=null, second=null;
	    	
	    	outer:
	        for (Disjunct d1 : newDisjuncts) {
	        	for (Disjunct d2 : newDisjuncts) {        		 
	            	for (Iterator<Map.Entry<String,Map>> posIter = ((Map<String,Map>)d1.posVarBinding).entrySet().iterator(); posIter.hasNext();) {
	            		Map.Entry<String,Map> entry = posIter.next();
	            		String var = entry.getKey();
	            		Map posBindingsD1 = entry.getValue();
	            		if(posBindingsD1==null) posBindingsD1 = new HashMap();
	            		Map negBindingsD2 = (Map) d2.negVarBinding.get(var);
	            		if(negBindingsD2==null) negBindingsD2 = new HashMap();
	            		Map intersection = new HashMap(posBindingsD1);
	            		intersection.keySet().retainAll(negBindingsD2.keySet());
	            		if(intersection.size()==1) {
	            			Disjunct d1Copy = d1.clone();
	            			Map posBindD1 = (Map) d1Copy.posVarBinding.get(var);	            			
	            			posBindD1.keySet().removeAll(intersection.keySet());
	            			if(posBindD1.isEmpty()) {
	            				d1Copy.posVarBinding.remove(var);
	            			}
	            			Disjunct d2Copy = d2.clone();
	            			Map negBindD2 = (Map) d2Copy.negVarBinding.get(var);	            			
	            			negBindD2.keySet().removeAll(intersection.keySet());
	            			if(negBindD2.isEmpty()) {
	            				d2Copy.negVarBinding.remove(var);
	            			}
	            			if(d1Copy.cloneWithoutHistory().equals(d2Copy.cloneWithoutHistory())) {
		            			superFlousKey = intersection.keySet().iterator().next();
		            			superFlousVar = var;
		            			first = d1;
		            			second = d2;
		            			break outer;
	    	...
	    	if(first!=null) {
	    		...
	    		Map iks = (Map) cloneOfFirst.posVarBinding.get(superFlousVar);
        ...
        for (Disjunct d1 : disjuncts) {
    		...
    		for (Iterator<Map.Entry<String,Map>> posIter = ((Map<String,Map>)d1.posVarBinding).entrySet().iterator(); posIter.hasNext();) {
            	...
            	for (Disjunct<InstanceKey> d2 : disjuncts) {        		 
                	...
                	Map negBindingsD2 = d2.negVarBinding.get(var);
                	//compare instance keys, not the history
                	if(negBindingsD2!=null && posBindingsD1.keySet().equals(negBindingsD2.keySet())) {
