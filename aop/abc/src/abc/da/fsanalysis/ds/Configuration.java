...
public class Configuration implements Cloneable {
	...
	public Configuration(TMFlowAnalysis flowAnalysis, Set<State> additionalInitialStates, boolean countFinalHits) {
		...
		this.tm = flowAnalysis.getTracePattern();
		...
		while(stateIter.hasNext()) {
			...
			boolean alwaysInInitialState = ((SimpleStateMachine)tm.getStateMachine()).alwaysInInitialState();
			if((alwaysInInitialState && state.isInitialNode()) || additionalInitialStates.contains(state))
	...
	public Configuration doTransition(Shadow shadow) {
		...
		Configuration tmp = getEmptyConfiguration();
		...
		final StateMachine sm = tm.getStateMachine();
...
//	}
	...
	/**
	 * Returns a fresh, empty configuration that is set to {@link Constraint#FALSE} everywhere. 
	 */
