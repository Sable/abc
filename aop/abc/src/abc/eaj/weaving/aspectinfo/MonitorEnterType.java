...
/**
 * Handler for <code>monitorenter</code> condition pointcut with a type
 * argument.
 * 
 * @author Eric Bodden
 */
public class MonitorEnterType extends MonitorEnterAny {
	private AbcType type;

	public MonitorEnterType(AbcType type, Position pos) {
		super(pos);
		this.type = type;
	}

	/**
	 * Get the type that is matched against the <code>monitorenter</code> argument by this
	 * <code>monitorenter</code> pointcut.
	 */
	...
	public boolean unify(Pointcut otherpc, Unification unification) {
		...
		if (otherpc.getClass() == this.getClass()) {
			AbcType othertype = ((MonitorEnterType) otherpc).getType();
			if (type.equals(othertype)) {
				unification.setPointcut(this);
				return true;
			} else
				return false;
		} else if ((otherpc.getClass() == MonitorEnterVar.class)
				&& (!unification.unifyWithFirst())) {
			// If the other pc is a ThisVar with the same type as this, can
			// unify
			// BUT only if we are not restricted to returning THIS
			if (abc.main.Debug.v().debugPointcutUnification)
				System.out.println("Trying to unify a ThisType " + this
						+ " with a ThisVar: " + otherpc);
			MonitorEnterVar othertv = (MonitorEnterVar) otherpc;
			if (this.getType().equals(
					unification.getType2(othertv.getVar().getName()))) {
				if (abc.main.Debug.v().debugPointcutUnification)
