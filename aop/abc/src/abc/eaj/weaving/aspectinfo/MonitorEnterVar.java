...
/**
 * Handler for <code>monitorenter</code> condition pointcut with a variable
 * argument.
 * 
 * @author Eric Bodden
 */
public class MonitorEnterVar extends MonitorEnterAny {
	protected Var var;
	protected WeavingVar weavingVar;

	public MonitorEnterVar(Var var, Position pos) {
		super(pos);
		this.var = var;
	}

	/**
	 * Get the pointcut variable that is bound by this <code>monitorenter</code>
	 * pointcut.
	 */
	public Var getVar() {
		return var;
	...
	public String toString() {
		return "monitorenter(" + var + ")";
	}

	protected Residue matchesAt(WeavingEnv we, ContextValue cv) {
		weavingVar = we.getWeavingVar(var);
		return Bind.construct(cv, we.getAbcType(var).getSootType(), weavingVar);
	}
	...
	public Pointcut inline(Hashtable renameEnv, Hashtable typeEnv,
		...
		if (var == this.var)
			...
			return new MonitorEnterVar(var, getPosition());
	}
	...
	public boolean unify(Pointcut otherpc, Unification unification) {
		...
		if (otherpc.getClass() == this.getClass()) {
			Var othervar = ((MonitorEnterVar) otherpc).getVar();
			if (var.unify(othervar, unification)) {
				...
				if (unifiedvar == var) {
					...
					if (unifiedvar == othervar) {
						...
						unification.setPointcut(new MonitorEnterVar(unifiedvar,
								unifiedvar.getPosition()));
						return true;
					}
				}
			} else
				return false;
		} else if (otherpc.getClass() == MonitorEnterType.class) {
			// If the other pc is a ThisType with the same type as this Var, can
			// unify
			if (abc.main.Debug.v().debugPointcutUnification)
				System.out.println("Trying to unify a MonitorEnterVar " + this
						+ " with a MonitorEnterType: " + otherpc);
			MonitorEnterType othertt = (MonitorEnterType) otherpc;
			if (unification.getType1(this.getVar().getName()).equals(
					othertt.getType())) {
				if (abc.main.Debug.v().debugPointcutUnification)
			...
			// Do the right thing if otherpc was a local vars pc
