...
/**
 * Handler for <code>monitorexit</code> condition pointcut with a variable
 * argument.
 * 
 * @author Eric Bodden
 */
public class MonitorExitVar extends MonitorExitAny {
	protected Var var;
	protected WeavingVar weavingVar;

	public MonitorExitVar(Var var, Position pos) {
		super(pos);
		this.var = var;
	}

	/**
	 * Get the pointcut variable that is bound by this <code>monitorexit</code>
	 * pointcut.
	 */
	public Var getVar() {
		return var;
	...
	public Pointcut inline(Hashtable renameEnv, Hashtable typeEnv,
		...
		if (var == this.var)
			...
			return new MonitorExitVar(var, getPosition());
	}
	...
	public boolean unify(Pointcut otherpc, Unification unification) {
		...
		if (otherpc.getClass() == this.getClass()) {
			Var othervar = ((MonitorExitVar) otherpc).getVar();
			if (var.unify(othervar, unification)) {
				...
				if (unifiedvar == var) {
					...
					if (unifiedvar == othervar) {
						...
						unification.setPointcut(new MonitorExitVar(unifiedvar,
				...
				return false;
		} else if (otherpc.getClass() == MonitorExitType.class) {
			// If the other pc is a ThisType with the same type as this Var, can
			// unify
			if (abc.main.Debug.v().debugPointcutUnification)
				System.out.println("Trying to unify a MonitorExitVar " + this
						+ " with a MonitorExitType: " + otherpc);
			MonitorExitType othertt = (MonitorExitType) otherpc;
