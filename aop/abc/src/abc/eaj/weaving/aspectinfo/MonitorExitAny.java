...
import abc.eaj.weaving.matching.MonitorExitShadowMatch;
...
/** Handler for <code>monitorexit</code> condition pointcut with a
 *  universal pattern argument.
 *  @author Eric Bodden
 */
public class MonitorExitAny extends DynamicValuePointcut {

    public MonitorExitAny(Position pos) {
        super(pos);
    }
    ...
    public final Residue matchesAt(MatchingContext mc) {
        ...
        if(!(sm instanceof MonitorExitShadowMatch))
        	return NeverMatch.v();
        
        Immediate syncValue = ((MonitorExitShadowMatch)sm).getSyncValue();
        return matchesAt(we,new JimpleValue(syncValue));
    }
