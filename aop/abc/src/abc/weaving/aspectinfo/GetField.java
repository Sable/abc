...
public class GetField extends ShadowPointcut {
    ...
    protected Residue matchesAt(ShadowMatch sm) {
        if(!(sm instanceof GetFieldShadowMatch)) return NeverMatch.v();
        SootFieldRef fieldref=((GetFieldShadowMatch) sm).getFieldRef();

        if(!getPattern().matchesFieldRef(fieldref)) return NeverMatch.v();
    ...
    public String toString() {
        ...
        public boolean unify(Pointcut otherpc, Unification unification) {
                ...
                if (otherpc.getClass() == this.getClass()) {
                        if (pattern.equivalent(((GetField)otherpc).getPattern())) {
