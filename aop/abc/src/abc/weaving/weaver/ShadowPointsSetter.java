...
public class ShadowPointsSetter {
    ...
    public void setShadowPoints(SootClass sc) {
      spsdebug(" --- BEGIN Setting ShadowPoints for " + sc.getName());
	 ...
	 // If there are any execution pointcuts
	 ShadowPoints execution_sp = null;
	 if (adviceList.bodyAdvice.size() > 0)  
	   { // transform body of method so it 
	     ...
	     spsdebug("Need to transform for execution in method: " +
		         method.getName()); 
	 ...
	 // --- Then look at constructor pointcuts, should only be for
	 //     methods called <init>
	 // TODO:  must make another weaving pass for initialization 
	 ShadowPoints preinitialization_sp = null;
	 if (adviceList.constructorAdvice.size() > 0)
	   { spsdebug("Need to transform for preinit in method: " +
	                  method.getName()); 
	     ...
	     if (method.getName().equals("<init>"))
	       ...
	       spsdebug("dealing with preinit of <init>");
	        ...
	        spsdebug("Ignoring preinit advice on method " +
		                                      method.getName());
	  }
	 ...
	 for (Iterator stmtlist = adviceList.stmtAdvice.iterator();
	      stmtlist.hasNext();)
		...
		//  stmtappl);
		spsdebug("Ignoring this kind of advice in method body: " +
		             stmtappl);
      ...
      spsdebug(" --- END Setting ShadowPoints for " + sc.getName());
    } // setShadowPoints
