...
public class PointcutCodeGen {
   ...
   public void weaveInAspectsPass( SootClass cl, int pass) {
	...
	(Local aspectref,AdviceApplication adviceappl, Chain units, 
	 LocalGeneratorEx localgen,WeavingContext wc) {
	   ...
	   debug("inserting at postion " + position + 
		 " into a Vector of size " + wc.arglist.capacity());
	   ...
	   { Stmt s = Jimple.v().
		     ...
		     newVirtualInvokeExpr( aspectref, advicemethod, wc.arglist));
             c.addLast(s);
	     return(c);
	   }
	   else
